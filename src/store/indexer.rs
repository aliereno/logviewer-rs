...
impl LogIndexer {
    pub fn new() -> Result<Self, Box<dyn Error>> {
        ...
        let index = match Index::create_in_dir(index_dir, schema.clone()) {
            Ok(i) => i,
            Err(e) => Index::open_in_dir(index_dir)?,
        }; 
        */
    ...
    pub fn search_logs(&self, source_id: i32, page: usize, page_size: usize) -> Result<Vec<serde_json::Map<std::string::String, serde_json::Value>>, Box<dyn Error>> {
        ...
        println!("search logs count {}", search_results.len());
